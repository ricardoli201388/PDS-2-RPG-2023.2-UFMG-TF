...
//char test = selectCharacter(player);
    ...
    mvprintw(5, 1, " Café da cura:%d ", inventario.curaCount);
    mvprintw(6, 1, " Pão de queijo de dano:%d ", inventario.danoCount);
...
void levelUp(Player& player) {
    ...
    mvprintw(27, 1, "Pressione qualquer tecla para continuar...");// Mensagem para continuar
          ...
          if(player.level== 3 ||player.level==  7 ){
              attron(A_BOLD);
              mvprintw(28, 1, "Você ganhou um café da cura");// Exibe mensagem de ganho de item: café da cura  para recuperar vida
              refresh();
          }
          if(player.level== 5 ||player.level== 9){
              attron(A_BOLD);
              mvprintw(28, 1, "Você ganhou um pão de queijo de dano");// Exibe mensagem de ganho de item: pão de queijo de dano
              refresh();
          }  
